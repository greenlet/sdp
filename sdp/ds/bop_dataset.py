...
class BatchParamsContainer(Generic[TBatchParams]):
    ...
    def __init__(self, params: TBatchParams):
...
def process_batch(params_cont: BatchParamsContainer) -> BatchResultContainer:
    try:
        worker_fn = load_imgs_objs_gt
        result = worker_fn(params_cont.params)
...
class BatchProcessor(Generic[TBatchParams, TBatchResult]):
    ...
    def sync_workers(self):
        ...
        for i in range(self.buffer_sz - n_sent - n_received):
            ...
            if params is None:
                break
            params_cont = BatchParamsContainer(params)
            ...
            print(f'apply_async batch_id = {params_cont.batch_id}')
    ...
    def get_batch_iterator(self, n_batches: Optional[int] = None, multiprocess: bool = False, batch_size: Optional[int] = None,
        ...
        def batch_gen() -> Generator[ImgsObjsGtParams, None, None]:
            for i_batch in range(n_batches):
                ...
                print(f'i_batch = {i_batch}. i = {i}. n_batches = {n_batches}')
                if i >= n:
